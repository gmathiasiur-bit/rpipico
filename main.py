...
async def conn_han(client):
    await client.subscribe('topico/temperaturaGMI', 1)
    await client.subscribe('topico/humedadGMI', 1)
    ...
    while True:
        try:
            ...
            try:
                ...
                await client.publish('topico/temperaturaGMI', '{}'.format(temperatura), qos = 1)
            except OSError as e:
                print("sin sensor temperatura")
            try:
                humedad=d.humidity()
                await client.publish('topico/humedadGMI', '{}'.format(humedad), qos = 1)
            except OSError as e:
